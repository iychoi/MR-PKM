...
import edu.arizona.cs.mrpkm.kmermatch.PairwiseKmerMatcherCmdParams;
import edu.arizona.cs.mrpkm.kmermatch.PairwiseKmerMatcherCmdParamsParser;
...
import edu.arizona.cs.mrpkm.kmermatch.KmerMatchInputFormat;
import edu.arizona.cs.mrpkm.kmermatch.KmerMatchInputFormatConfig;
        ...
        for(int i=0;i<groups.length;i++) {
            ...
            for(int j=0;j<groups.length;j++) {
                ...
                if(i != j) {
                    ...
                    namedOutputs.add(pairwiseModeCounterOutputName);
                }
            }
        }
        ...
        if (status.isDir()) {
            ...
            for (FileStatus entry : entries) {
                ...
                if(MapReduceHelper.isLogFiles(entryPath)) {
                    ...
                    if(namedOutput != null) {
                        Path toPath = new Path(entryPath.getParent(), PairwiseKmerModeCounterHelper.makePairwiseModeCountFileName(namedOutput.getFilename()));
